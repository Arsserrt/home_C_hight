...
{
    // Приводим букву к нижнему регистру (для W/A/S/D)
    int32_t lower_key = key;
    if (key >= 'A' && key <= 'Z')
        lower_key = key - 'A' + 'a';
    
        //сама проверка
    if (snake->direction == DOWN) if (key == snake->controls.up  || lower_key == snake->controls.up_alt) return 1;
    if (snake->direction == UP) if (key == snake->controls.down  || lower_key == snake->controls.down_alt) return 1;
    if (snake->direction == RIGHT) if (key == snake->controls.left  || lower_key == snake->controls.left_alt) return 1;
     if (snake->direction == LEFT) if (key == snake->controls.right || lower_key == snake->controls.right_alt) return 1;
    return 0;
}
...
void changeDirection(struct snake_t* snake, const int32_t key)
{
    ...
    if (key == snake->controls.down  || lower_key == snake->controls.down_alt)
        snake->direction = DOWN;
    else if (key == snake->controls.up    || lower_key == snake->controls.up_alt)
        snake->direction = UP;
    else if (key == snake->controls.right || lower_key == snake->controls.right_alt)
        snake->direction = RIGHT;
    else if (key == snake->controls.left  || lower_key == snake->controls.left_alt)
...
*/
    ...
    while( key_pressed != STOP_GAME )
    {
        ...
        if(checkDirection(snake, key_pressed)) key_pressed=0;
        go(snake);
        snakeFail(snake);
        goTail(snake);
